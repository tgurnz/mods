...
#============================================================= 
<#
CL_RemoveOffice2016.ps1 uninstall office 2016 c2r or MSI
...
#*=================================================================================
Import-LocalizedData -BindingVariable Strings_CL_RemoveOffice2016 -FileName CL_RemoveOffice2016

#Get the size of the OSArchitecure to check whether the office installed in 32bit or 64 bit OS.
$osArchitecture = [IntPtr]::Size;

Function Remove-Office2016($commonFilesRegx86,$commonFilesReg,$officeIDList,$programFilesReg,$programFilesx86Reg,$oSVersion,$count)
{
	<#
	Remove-Office2016
	...
	if($officeIDList -eq "Office16C2R")
	{
		...
		foreach($getprocesslist in $processList)
		{
			...
			if($process)
			{
				Stop-Process -Name $getprocesslist -force -ErrorAction SilentlyContinue
			}
		}	
	...
	$OfficeRegID = "16.0"
	$officeProductID = "*000061*"
	$officeHKCRID = @("Products","Features","UpgradeCodes")
	$officeThemes15 = $commonFilesReg  + "\microsoft shared\Themes16"
	$officethemes15X86 = $commonFilesRegx86  + "\microsoft shared\Themes16"
	...
	$officestartMenushortcut = $env:ProgramData + "\Microsoft\Windows\Start Menu\Programs"
	...
	$officeGuid = "{9116"
	$officeGuidId = "{9016"
	$officeVersionID = "2016"
	$officec2r1516 = "Office 16"
	$hkcrkeys = @()
	$controlpanelregList = @()
	$customPath = @()
	...
	if($osArchitecture -eq 8)
	{
     ...
     #get Office 2016 related registries & directories...
	 $deleteOffice2003Paths = @("$clickToRunSvc","$osppsvc","$ose64","$officeThemes15","$officethemes15X86","$officecommonfilesx86","$officecommonfiles","$officesoftwareProtectionx86","$officesoftwareProtection","$appV","$clickToRun","$clickToRunShared","$clickToRunSharedx86")
     $pathlist = @()
     $pathlist += $hkcrkeys
	 $pathlist += $customPath 
	 $pathlist += $deleteOffice2003Paths
	 # call functions to remove registries and directories
     Remove-OfficeRegistry $controlpanelregList
     Remove-OfficeDirectory $pathlist	
	 Get-ChildItem $officestartMenushortcut | Where-Object {$_.name -like "*2016*"} | Remove-Item -Recurse -Force
}

#Call the function to remove office 2016
Remove-Office2016 $commonFilesRegx86 $commonFilesReg $officeID $programFilesReg $programFilesx86Reg $oSVersion $count

# SIG # Begin signature block
# MIIdqAYJKoZIhvcNAQcCoIIdmTCCHZUCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUBxJy3jQVmnYQA6kzczToTmi3
# i+CgghhkMIIEwzCCA6ugAwIBAgITMwAAAMWWQGBL9N6uLgAAAAAAxTANBgkqhkiG
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUMRSf4wpy/FnvPBum3oGFRQNI35kwYgYKKwYB
# BAGCNwIBDDFUMFKgOIA2AE8AVQA0AF8AQwBMAF8AUgBlAG0AbwB2AGUATwBmAGYA
# aQBjAGUAMgAwADEANgAuAHAAcwAxoRaAFGh0dHA6Ly9taWNyb3NvZnQuY29tMA0G
# CSqGSIb3DQEBAQUABIIBACbKC0KM5b1rerIhnI/5Encjm9Bb4hUWnUByNqNgnk6n
# G/BpBvNDwQN8JPCC7fXB7bqANycbEmC1j14PdlI80MF4NkbGpUX/LPhFQFNhOHn6
# 5iRqMjpne4OHhacckP4Z0Qmkm8MLh/BmfZpb5rYdYZlMewnWgo3L78LIr1c2c2jK
# MBoOJltUVILvjNTH9AZa7zvJdPH4LOeIvmK9TMXBaJZeaNPZitWcxqQUOdF5AYDz
# +DjWPMjOzS7yo1ddOVc+lwpbTdPLngRKSOyNfdaVCjDi/tffk4frP7g9MzN4UCBn
# Cw402lzgs8Akk1HiMQLKhfZMgSKMZ7npiLdI+Jx7WXuhggIoMIICJAYJKoZIhvcN
...
# BDEWBBTMcFF2S02Hh1bO8ra35ZFrhhcqDTANBgkqhkiG9w0BAQUFAASCAQA1clCS
# oq6YcXj3DFolqdsLjAB3lGBx1vc4vw4E9wZCquqdKM2bN3SjbOuehqgWQyMECdbY
# zkG4tLm+EFPS4x8ejNkJMjGs1Bzmgp9KaVCjgHCYZNzuHMh1frz2LBVcUtdsYemW
# MTAgp43NkXtjwGAb7LzOTEigXEO1ERbXTdTk9WxGixNdWMP+X2qooC+1jmq6laul
# ZDmgoxz5kNlBacgXbtT/ftJ+nAfW82bJAmWA/xQ3f5VjCeZE5Kdwa9mjoyXaY9g8
# tlD22gwQzVhbDoJMtTkzfWf9YJOl3DCPWN3O/R/2SomS7Dw/vgIHs4OL52zvAQ0b
# b8mfRjVBbwIF9j2W
